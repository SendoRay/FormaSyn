...
--        bit 7~0 - x_in[7:0] (Read/Write)
--        others  - reserved
-- 0x14 : reserved
-- 0x18 : Data signal of y_out_scale
--        bit 7~0 - y_out_scale[7:0] (Read)
--        others  - reserved
-- 0x1c : reserved
...
architecture behave of kernel_control_s_axi is
    ...
    constant ADDR_Y_OUT_SCALE_DATA_0 : INTEGER := 16#18#;
    constant ADDR_Y_OUT_SCALE_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_x_in            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_y_out_scale     : UNSIGNED(7 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_X_IN_DATA_0 =>
                        rdata_data <= RESIZE(int_x_in(7 downto 0), 32);
                    when ADDR_Y_OUT_SCALE_DATA_0 =>
                        rdata_data <= RESIZE(int_y_out_scale(7 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_x_in(7 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_X_IN_DATA_0) then
                    int_x_in(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_x_in(7 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_y_out_scale <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (ap_done = '1') then
                    int_y_out_scale <= UNSIGNED(y_out_scale);
                end if;
            end if;
        end if;
    end process;
