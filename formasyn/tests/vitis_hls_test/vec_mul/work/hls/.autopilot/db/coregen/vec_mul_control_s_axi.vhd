...
entity vec_mul_control_s_axi is
generic (
...
end entity vec_mul_control_s_axi;
...
architecture behave of vec_mul_control_s_axi is
    ...
    constant ADDR_Y_BASE  : INTEGER := 16#40#;
    constant ADDR_Y_HIGH  : INTEGER := 16#5f#;
    ...
    end component vec_mul_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_a
int_a : vec_mul_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_a_q1);
-- int_b
int_b : vec_mul_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_b_q1);
-- int_y
int_y : vec_mul_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "S2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- Memory logic ------------------
    ...
    -- y
    int_y_address0       <= RESIZE(UNSIGNED(y_address0), 3);
    int_y_ce0            <= y_ce0;
    int_y_be0            <= (others => y_we0);
    int_y_d0             <= RESIZE(UNSIGNED(y_d0), 32);
    int_y_address1       <= raddr(4 downto 2);
...
entity vec_mul_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity vec_mul_control_s_axi_ram;

architecture behave of vec_mul_control_s_axi_ram is
