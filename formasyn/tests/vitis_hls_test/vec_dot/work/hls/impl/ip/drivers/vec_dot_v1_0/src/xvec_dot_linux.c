...
typedef struct {
    ...
    XVec_dot_uio_map maps[ MAX_UIO_MAPS ];
} XVec_dot_uio_info;

/***************** Variable Definitions **************************************/
static XVec_dot_uio_info uio_info;
...
static int uio_info_read_name(XVec_dot_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVec_dot_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVec_dot_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVec_dot_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVec_dot_Initialize(XVec_dot *InstancePtr, const char* InstanceName) {
	XVec_dot_uio_info *InfoPtr = &uio_info;
