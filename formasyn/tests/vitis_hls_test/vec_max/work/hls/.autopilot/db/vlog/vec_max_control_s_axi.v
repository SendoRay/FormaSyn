    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IDX_DATA_0: begin
                    rdata <= int_idx[2:0];
                end
                ADDR_IDX_CTRL: begin
                    rdata[0] <= int_idx_ap_vld;
                end
            endcase
        end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (idx_ap_vld)
            int_idx_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_IDX_CTRL)
            int_idx_ap_vld <= 1'b0; // clear on read
    end
end
...
module vec_max_control_s_axi_ram
#(parameter
    MEM_STYLE  = "auto",
    MEM_TYPE   = "S2P",
    BYTE_WIDTH = 8,
