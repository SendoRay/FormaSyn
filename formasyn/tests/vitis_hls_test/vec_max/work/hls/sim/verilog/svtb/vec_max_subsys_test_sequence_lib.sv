...
//
//==============================================================
`ifndef VEC_MAX_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define VEC_MAX_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class vec_max_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "vec_max_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(vec_max_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(vec_max_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            if (!uvm_config_db#(vec_max_reference_model)::get(p_sequencer,"", "refm", refm))
            ...
            fork
                    ...
                    fork
                        begin
                            ...
                            for(int i=0; i<3; i++) begin
                                ...
                                `uvm_info("control data writting thread", $sformatf("%0dth(total 3): waiting for all write data finish event",i), UVM_LOW)
                                ...
                                fork
                                    begin
                                        ...
                                        `uvm_info("control start dut by axilite", $sformatf("%0dth(total 3): begin to set start bit",i), UVM_LOW)
                                        `uvm_send(axi_master_wr_control_seq);
                                    end
                                    begin
                                        `uvm_info("control wait for ap_ready for next trans", $sformatf("%0dth(total 3): begin to wait",i), UVM_LOW)
                        ...
                        begin
                            for(int j=0; j<3; j=j+refm.ap_done_cnt) begin
                                ...
                                fork
                                    ...
                                    begin
                                        `uvm_info("test finish control", $sformatf("%0dth(total 3) ap_done_for_nexttrans begin to wait",j), UVM_LOW)
                                        @refm.dut2tb_ap_done;
                                    end
                                join_any
                                disable fork;
                        ...
                        begin
                            ...
                            for(int j=0; j<3; j=j+refm.ap_done_cnt) begin
                                ...
                                axi_master_rd_control_seq.datamerge_inavg(databusbit, (1*4+32-1)/32, 48, 0);
                                    `uvm_info("axilite data read", $sformatf("%0dth(total 3) data read sequence is started",j), UVM_LOW)
                                ...
                                `uvm_info("continue bit set by axilite", $sformatf("%0dth(total 3) continue setting is started",j), UVM_LOW)
                ...
                begin
                    for(int j=0; j<3; j=j+refm.ap_done_cnt) @refm.ap_done_for_nexttrans;
