...
entity vec_max_control_s_axi is
generic (
...
end entity vec_max_control_s_axi;
...
architecture behave of vec_max_control_s_axi is
    ...
    constant ADDR_IDX_DATA_0 : INTEGER := 16#30#;
    constant ADDR_IDX_CTRL   : INTEGER := 16#34#;
    ...
    signal int_idx_ap_vld      : STD_LOGIC;
    signal int_idx             : UNSIGNED(2 downto 0) := (others => '0');
    -- memory signals
    ...
    component vec_max_control_s_axi_ram is
        generic (
    ...
    end component vec_max_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_a
int_a : vec_max_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_a_read = '0') else '0';
    RVALID    <= RVALID_t;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_Y_CTRL =>
                        rdata_data(0) <= int_y_ap_vld;
                    when ADDR_IDX_DATA_0 =>
                        rdata_data <= RESIZE(int_idx(2 downto 0), 32);
                    when ADDR_IDX_CTRL =>
                        rdata_data(0) <= int_idx_ap_vld;
                    when others =>
                        NULL;
                    end case;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (idx_ap_vld = '1') then
                    int_idx <= UNSIGNED(idx);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (idx_ap_vld = '1') then
                    int_idx_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_IDX_CTRL) then
                    int_idx_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
...
entity vec_max_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity vec_max_control_s_axi_ram;

architecture behave of vec_max_control_s_axi_ram is
