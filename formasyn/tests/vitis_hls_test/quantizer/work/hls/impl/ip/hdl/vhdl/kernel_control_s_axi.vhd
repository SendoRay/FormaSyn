...
architecture behave of kernel_control_s_axi is
    ...
    constant ADDR_X_IN_CTRL    : INTEGER := 16#14#;
    constant ADDR_BITS_DATA_0  : INTEGER := 16#18#;
    constant ADDR_BITS_CTRL    : INTEGER := 16#1c#;
    constant ADDR_FRAC_DATA_0  : INTEGER := 16#20#;
    constant ADDR_FRAC_CTRL    : INTEGER := 16#24#;
    constant ADDR_Y_OUT_DATA_0 : INTEGER := 16#28#;
    constant ADDR_Y_OUT_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_x_in            : UNSIGNED(31 downto 0) := (others => '0');
    signal int_bits            : UNSIGNED(4 downto 0) := (others => '0');
    signal int_frac            : UNSIGNED(4 downto 0) := (others => '0');
    signal int_y_out           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_BITS_DATA_0 =>
                        rdata_data <= RESIZE(int_bits(4 downto 0), 32);
                    when ADDR_FRAC_DATA_0 =>
                        rdata_data <= RESIZE(int_frac(4 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    bits                 <= STD_LOGIC_VECTOR(int_bits);
    frac                 <= STD_LOGIC_VECTOR(int_frac);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_bits(4 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_BITS_DATA_0) then
                    int_bits(4 downto 0) <= (UNSIGNED(WDATA(4 downto 0)) and wmask(4 downto 0)) or ((not wmask(4 downto 0)) and int_bits(4 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_frac(4 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FRAC_DATA_0) then
                    int_frac(4 downto 0) <= (UNSIGNED(WDATA(4 downto 0)) and wmask(4 downto 0)) or ((not wmask(4 downto 0)) and int_frac(4 downto 0));
                end if;
            end if;
        end if;
    end process;
