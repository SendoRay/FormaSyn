...
#(parameter
    ...
    output wire [31:0]                   x_in,
    output wire [4:0]                    bits,
    output wire [4:0]                    frac,
    input  wire [31:0]                   y_out,
...
localparam
    ...
    ADDR_X_IN_CTRL    = 6'h14,
    ADDR_BITS_DATA_0  = 6'h18,
    ADDR_BITS_CTRL    = 6'h1c,
    ADDR_FRAC_DATA_0  = 6'h20,
    ADDR_FRAC_CTRL    = 6'h24,
    ADDR_Y_OUT_DATA_0 = 6'h28,
    ADDR_Y_OUT_CTRL   = 6'h2c,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_x_in = 'b0;
    reg  [4:0]                    int_bits = 'b0;
    reg  [4:0]                    int_frac = 'b0;
    reg  [31:0]                   int_y_out = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BITS_DATA_0: begin
                    rdata <= int_bits[4:0];
                end
                ADDR_FRAC_DATA_0: begin
                    rdata <= int_frac[4:0];
                end
...
assign bits          = int_bits;
assign frac          = int_frac;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BITS_DATA_0)
            int_bits[4:0] <= (WDATA[31:0] & wmask) | (int_bits[4:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_FRAC_DATA_0)
            int_frac[4:0] <= (WDATA[31:0] & wmask) | (int_frac[4:0] & ~wmask);
    end
end
