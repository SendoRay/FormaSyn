    ...
    class kernel_subsys_test_sequence_lib extends uvm_sequence;                                
        ...
        virtual task body();                                                                        
            ...
            fork
                begin
                    fork
                        begin
                            ...
                            for(int i=0; i<5004; i++) begin
                                logic[63:0] data64bit_x_in[$];
                                logic[32-1:0] databusbit_x_in[$];
                                logic[63:0] data64bit_bits[$];
                                logic[32-1:0] databusbit_bits[$];
                                data64bit_x_in.delete(); databusbit_x_in.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_control_x_in.tobusdata(data64bit_x_in, refm.mem_blk_pages_control_x_in.rd_page_idx, 32);
                                foreach(data64bit_x_in[s]) databusbit_x_in[s]=data64bit_x_in[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_x_in, 0, 16, 1);
                                data64bit_bits.delete(); databusbit_bits.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_control_bits.tobusdata(data64bit_bits, refm.mem_blk_pages_control_bits.rd_page_idx, 32);
                                foreach(data64bit_bits[s]) databusbit_bits[s]=data64bit_bits[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_bits, 0, 24, 1);
                                ...
                                `uvm_info("control data writting thread", $sformatf("%0dth(total 5004): waiting for all write data finish event",i), UVM_LOW)
                                ...
                                fork
                                    begin
                                        ...
                                        `uvm_info("control start dut by axilite", $sformatf("%0dth(total 5004): begin to set start bit",i), UVM_LOW)
                                        `uvm_send(axi_master_wr_control_seq);
                                    end
                                    begin
                                        `uvm_info("control wait for ap_ready for next trans", $sformatf("%0dth(total 5004): begin to wait",i), UVM_LOW)
                        ...
                        begin
                            for(int j=0; j<5004; j=j+refm.ap_done_cnt) begin
                                ...
                                fork
                                    ...
                                    begin
                                        `uvm_info("test finish control", $sformatf("%0dth(total 5004) ap_done_for_nexttrans begin to wait",j), UVM_LOW)
                                        @refm.dut2tb_ap_done;
                                    end
                                join_any
                                disable fork;
                        ...
                        begin
                            ...
                            for(int j=0; j<5004; j=j+refm.ap_done_cnt) begin
                                ...
                                @refm.dut2tb_ap_done;
                                axi_master_rd_control_seq.datamerge_inavg(databusbit, (1*32+32-1)/32, 40, 0);
                                    `uvm_info("axilite data read", $sformatf("%0dth(total 5004) data read sequence is started",j), UVM_LOW)
                                ...
                                `uvm_info("continue bit set by axilite", $sformatf("%0dth(total 5004) continue setting is started",j), UVM_LOW)
                ...
                begin
                    for(int j=0; j<5004; j=j+refm.ap_done_cnt) @refm.ap_done_for_nexttrans;
