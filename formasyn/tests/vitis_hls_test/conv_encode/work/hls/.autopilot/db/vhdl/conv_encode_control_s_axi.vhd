...
entity conv_encode_control_s_axi is
generic (
...
end entity conv_encode_control_s_axi;
...
--        others - reserved
-- 0x10 : Data signal of num_bits
--        bit 31~0 - num_bits[31:0] (Read/Write)
-- 0x14 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of conv_encode_control_s_axi is
    ...
    constant ADDR_NUM_BITS_DATA_0 : INTEGER := 16#10#;
    constant ADDR_NUM_BITS_CTRL   : INTEGER := 16#14#;
    ...
    signal int_num_bits        : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUM_BITS_DATA_0 =>
                        rdata_data <= RESIZE(int_num_bits(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    num_bits             <= STD_LOGIC_VECTOR(int_num_bits);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_num_bits(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUM_BITS_DATA_0) then
                    int_num_bits(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_num_bits(31 downto 0));
                end if;
            end if;
        end if;
    end process;
