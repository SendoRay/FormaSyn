...
`ifndef CONV_ENCODE_REFERENCE_MODEL_SV
`define CONV_ENCODE_REFERENCE_MODEL_SV

class conv_encode_reference_model extends uvm_component;
`define TV_IN_num_bits "../tv/cdatafile/c.conv_encode.autotvin_num_bits.dat"
`define TV_OUT_num_bits ""
    ...
    mem_model_pages#(32,8) mem_blk_pages_control_num_bits;
    int svr_bit_in_stream_delay;
    covergroup svr_bit_in_stream_cov;
        delay: coverpoint svr_bit_in_stream_delay
        {
            bins norm[2] = { [0 : 1] };
        }
    endgroup
    int svr_y_out_stream_delay;
    covergroup svr_y_out_stream_cov;
        delay: coverpoint svr_y_out_stream_delay
        {
            bins norm[2] = { [0 : 1] };
        }
    endgroup
    
    `uvm_component_utils_begin(conv_encode_reference_model)
    ...
    function new (string name = "", uvm_component parent = null);
        ...
        svr_bit_in_stream_cov = new;
        svr_y_out_stream_cov = new;
        trans_num_idx= 0;
    endfunction
...
misc_if.dut2tb_ap_done = 0;
        ...
        fpath.push_back(`TV_IN_num_bits);
        mem_blk_pages_control_num_bits = mem_model_pages#(32,8)::type_id::create("mem_blk_pages_control_num_bits");
        mem_blk_pages_control_num_bits.tvinload_pagechk_atinit(fpath, 1*((32+7)/8), 0, 16, "");
        fpath.delete;
    ...
    //  trans_size++;
        svr_bit_in_stream_delay = tr.delay;
        svr_bit_in_stream_cov.sample();
    ...
    //  trans_size++;
        svr_y_out_stream_delay = tr.delay;
        svr_y_out_stream_cov.sample();
        `uvm_info(this.get_full_name(), "port a collected one pkt", UVM_DEBUG);
    endfunction
