...
# 5 "./kernel.h" 2
# 20 "./kernel.h"
__attribute__((sdx_kernel("kernel", 0))) void kernel(ap_int<16> x_in, ap_int<16>& y_out);
# 2 "kernel.cpp" 2
# 1 "/tools/Xilinx/2025.1/Vitis/common/technology/autopilot/ap_int.h" 1
# 3 "kernel.cpp" 2
# 12 "kernel.cpp"
...
#pragma HLS PIPELINE II=1
 ...
 const int TOTAL_TAPS = 17;
    const int NUM_NONZERO = 9;
    const int CENTER_TAP = 8;
...
#pragma HLS ARRAY_PARTITION variable=shift_reg complete
 ...
 VITIS_LOOP_46_1: for (int i = TOTAL_TAPS - 1; i > 0; i--) {
#pragma HLS UNROLL
 shift_reg[i] = shift_reg[i-1];
    ...
    ap_int<17> pairs[NUM_NONZERO/2 + 1];
#pragma HLS ARRAY_PARTITION variable=pairs complete


 pairs[0] = shift_reg[0] + shift_reg[16];
    pairs[1] = shift_reg[2] + shift_reg[14];
    pairs[2] = shift_reg[4] + shift_reg[12];
    pairs[3] = shift_reg[6] + shift_reg[10];
    pairs[4] = shift_reg[8];
...
#pragma HLS ARRAY_PARTITION variable=products complete
 ...
 products[0] = pairs[0] * (-21);
    ...
    ap_int<40> acc = 0;
    VITIS_LOOP_85_2: for (int i = 0; i < 5; i++) {
#pragma HLS UNROLL
 acc += products[i];
    }
