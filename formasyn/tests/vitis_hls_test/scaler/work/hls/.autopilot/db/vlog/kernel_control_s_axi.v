...
#(parameter
    ...
    output wire [15:0]                   x_in,
    output wire [15:0]                   gain,
    input  wire [15:0]                   y_out,
...
localparam
    ...
    ADDR_GAIN_DATA_0  = 6'h18,
    ADDR_GAIN_CTRL    = 6'h1c,
    ADDR_Y_OUT_DATA_0 = 6'h20,
    ADDR_Y_OUT_CTRL   = 6'h24,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_x_in = 'b0;
    reg  [15:0]                   int_gain = 'b0;
    reg  [15:0]                   int_y_out = 'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_X_IN_DATA_0)
            int_x_in[15:0] <= (WDATA[31:0] & wmask) | (int_x_in[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_GAIN_DATA_0)
            int_gain[15:0] <= (WDATA[31:0] & wmask) | (int_gain[15:0] & ~wmask);
    end
end
