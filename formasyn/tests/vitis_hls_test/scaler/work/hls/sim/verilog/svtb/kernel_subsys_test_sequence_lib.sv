    ...
    class kernel_subsys_test_sequence_lib extends uvm_sequence;                                
        ...
        virtual task body();                                                                        
            ...
            fork
                begin
                    fork
                        begin
                            ...
                            for(int i=0; i<4005; i++) begin
                                logic[63:0] data64bit_x_in[$];
                                logic[32-1:0] databusbit_x_in[$];
                                logic[63:0] data64bit_gain[$];
                                logic[32-1:0] databusbit_gain[$];
                                ...
                                data64bit_gain.delete(); databusbit_gain.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_control_gain.tobusdata(data64bit_gain, refm.mem_blk_pages_control_gain.rd_page_idx, 32);
                                foreach(data64bit_gain[s]) databusbit_gain[s]=data64bit_gain[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_gain, 0, 24, 1);
                                ...
                                `uvm_info("control data writting thread", $sformatf("%0dth(total 4005): waiting for all write data finish event",i), UVM_LOW)
                                ...
                                fork
                                    begin
                                        ...
                                        `uvm_info("control start dut by axilite", $sformatf("%0dth(total 4005): begin to set start bit",i), UVM_LOW)
                                        `uvm_send(axi_master_wr_control_seq);
                                    end
                                    begin
                                        `uvm_info("control wait for ap_ready for next trans", $sformatf("%0dth(total 4005): begin to wait",i), UVM_LOW)
                        ...
                        begin
                            for(int j=0; j<4005; j=j+refm.ap_done_cnt) begin
                                ...
                                fork
                                    ...
                                    begin
                                        `uvm_info("test finish control", $sformatf("%0dth(total 4005) ap_done_for_nexttrans begin to wait",j), UVM_LOW)
                                        @refm.dut2tb_ap_done;
                                    end
                                join_any
                                disable fork;
                        ...
                        begin
                            ...
                            for(int j=0; j<4005; j=j+refm.ap_done_cnt) begin
                                ...
                                axi_master_rd_control_seq.datamerge_inavg(databusbit, (1*16+32-1)/32, 32, 0);
                                    `uvm_info("axilite data read", $sformatf("%0dth(total 4005) data read sequence is started",j), UVM_LOW)
                                ...
                                `uvm_info("continue bit set by axilite", $sformatf("%0dth(total 4005) continue setting is started",j), UVM_LOW)
                ...
                begin
                    for(int j=0; j<4005; j=j+refm.ap_done_cnt) @refm.ap_done_for_nexttrans;
