...
architecture behave of kernel_control_s_axi is
    ...
    constant ADDR_GAIN_DATA_0  : INTEGER := 16#18#;
    constant ADDR_GAIN_CTRL    : INTEGER := 16#1c#;
    constant ADDR_Y_OUT_DATA_0 : INTEGER := 16#20#;
    constant ADDR_Y_OUT_CTRL   : INTEGER := 16#24#;
    ...
    signal int_x_in            : UNSIGNED(15 downto 0) := (others => '0');
    signal int_gain            : UNSIGNED(15 downto 0) := (others => '0');
    signal int_y_out           : UNSIGNED(15 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_X_IN_DATA_0 =>
                        rdata_data <= RESIZE(int_x_in(15 downto 0), 32);
                    when ADDR_GAIN_DATA_0 =>
                        rdata_data <= RESIZE(int_gain(15 downto 0), 32);
                    when ADDR_Y_OUT_DATA_0 =>
                        rdata_data <= RESIZE(int_y_out(15 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_x_in(15 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_X_IN_DATA_0) then
                    int_x_in(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_x_in(15 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_gain(15 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_GAIN_DATA_0) then
                    int_gain(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_gain(15 downto 0));
                end if;
            end if;
        end if;
    end process;
