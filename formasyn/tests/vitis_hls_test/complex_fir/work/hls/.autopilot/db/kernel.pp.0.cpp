...
# 5 "./kernel.h" 2
# 25 "./kernel.h"
__attribute__((sdx_kernel("kernel", 0))) void kernel(ap_int<16> x_real, ap_int<16> x_imag, ap_int<16>& y_real, ap_int<16>& y_imag);
# 2 "kernel.cpp" 2
# 1 "/tools/Xilinx/2025.1/Vitis/common/technology/autopilot/ap_int.h" 1
# 3 "kernel.cpp" 2
# 20 "kernel.cpp"
...
#pragma HLS ARRAY_PARTITION variable=shift_reg_imag complete
 ...
 VITIS_LOOP_58_1: for (int i = TAPS - 1; i > 0; i--) {
#pragma HLS UNROLL
 shift_reg_real[i] = shift_reg_real[i-1];
    ...
    VITIS_LOOP_72_2: for (int i = 0; i < TAPS; i++) {
#pragma HLS UNROLL

 prod_t prod_i_hr = shift_reg_real[i] * coeffs_real[i];
        prod_t prod_q_hi = shift_reg_imag[i] * coeffs_imag[i];
        prod_t prod_i_hi = shift_reg_real[i] * coeffs_imag[i];
        prod_t prod_q_hr = shift_reg_imag[i] * coeffs_real[i];
    ...
    data_t max_val = 32767;
    data_t min_val = -32768;
