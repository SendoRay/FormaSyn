...
entity ofdm_base_control_s_axi is
generic (
...
end entity ofdm_base_control_s_axi;
...
architecture behave of ofdm_base_control_s_axi is
    ...
    constant ADDR_IN_R_BASE  : INTEGER := 16#40#;
    constant ADDR_IN_R_HIGH  : INTEGER := 16#7f#;
    constant ADDR_OUT_R_BASE : INTEGER := 16#80#;
    constant ADDR_OUT_R_HIGH : INTEGER := 16#ff#;
    ...
    -- memory signals
    signal int_in_r_address0   : UNSIGNED(3 downto 0);
    signal int_in_r_ce0        : STD_LOGIC;
    signal int_in_r_q0         : UNSIGNED(31 downto 0);
    signal int_in_r_address1   : UNSIGNED(3 downto 0);
    ...
    component ofdm_base_control_s_axi_ram is
        generic (
    ...
    end component ofdm_base_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_in_r
int_in_r : ofdm_base_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_in_r_q1);
-- int_out_r
int_out_r : ofdm_base_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "S2P",
     BYTE_WIDTH => 8,
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    rdata_data <= RESIZE(int_in_r_q1, 32);
                elsif (int_out_r_read = '1') then
                    rdata_data <= RESIZE(int_out_r_q1, 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- in_r
    int_in_r_address0    <= RESIZE(UNSIGNED(in_r_address0), 4);
    int_in_r_ce0         <= in_r_ce0;
    in_r_q0              <= STD_LOGIC_VECTOR(RESIZE(int_in_r_q0, 32));
    int_in_r_address1    <= raddr(5 downto 2) when ar_hs = '1' else waddr(5 downto 2);
    ...
    int_in_r_be1         <= UNSIGNED(WSTRB) when int_in_r_we1 = '1' else (others=>'0');
    int_in_r_d1          <= UNSIGNED(WDATA);
    -- out_r
    int_out_r_address0   <= RESIZE(UNSIGNED(out_r_address0), 5);
    int_out_r_ce0        <= out_r_ce0;
    int_out_r_be0        <= (others => out_r_we0);
    int_out_r_d0         <= RESIZE(UNSIGNED(out_r_d0), 32);
    int_out_r_address1   <= raddr(6 downto 2);
    int_out_r_ce1        <= '1' when ar_hs = '1' else '0';
...
entity ofdm_base_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity ofdm_base_control_s_axi_ram;

architecture behave of ofdm_base_control_s_axi_ram is
