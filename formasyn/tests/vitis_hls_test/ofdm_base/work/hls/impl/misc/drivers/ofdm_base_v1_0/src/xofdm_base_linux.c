...
typedef struct {
    ...
    XOfdm_base_uio_map maps[ MAX_UIO_MAPS ];
} XOfdm_base_uio_info;

/***************** Variable Definitions **************************************/
static XOfdm_base_uio_info uio_info;
...
static int uio_info_read_name(XOfdm_base_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOfdm_base_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOfdm_base_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOfdm_base_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOfdm_base_Initialize(XOfdm_base *InstancePtr, const char* InstanceName) {
	XOfdm_base_uio_info *InfoPtr = &uio_info;
