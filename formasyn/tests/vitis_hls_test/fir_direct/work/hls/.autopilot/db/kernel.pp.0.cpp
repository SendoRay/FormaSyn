...
#pragma HLS ARRAY_PARTITION variable=shift_reg complete
 ...
 VITIS_LOOP_42_1: for (int i = TAPS - 1; i > 0; i--) {
#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION variable=products complete
 ...
 VITIS_LOOP_52_2: for (int i = 0; i < TAPS; i++) {
#pragma HLS UNROLL
 products[i] = shift_reg[i] * coeffs[i];
    ...
    acc_t acc = 0;
    VITIS_LOOP_59_3: for (int i = 0; i < TAPS; i++) {
#pragma HLS UNROLL
 acc += products[i];
    }
    ...
    ap_int<DATA_WIDTH> max_val = (1 << (DATA_WIDTH - 1)) - 1;
    ap_int<DATA_WIDTH> min_val = -(1 << (DATA_WIDTH - 1));
