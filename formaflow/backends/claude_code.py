...
out to `claude -p` (pipe mode) to perform completions.
"""
...
class ClaudeCodeBackend:
    ...
    def _call_cli(self, prompt: str) -> str:
        ...
        try:
            # Use cat pipe to avoid argument length limits
            ...
            if result.returncode != 0:
                logger.error("claude CLI error (rc=%d): %s", result.returncode, result.stderr[:500])
                return ""
